...
- Hashing user-supplied API keys isn't useful because hashing is one-way; the application needs the raw key to call the provider. If the goal is to avoid raw persistence, prefer not persisting the key and instruct the user to set an env var or add a file to their home directory.
- For an improved UX later, integrate OS keychain storage and a short command that stores the key there (native). For now, focus on clear instructions and config file + env var.

---

If you confirm this spec, I can next draft Java interface skeletons and the small `SecretResolver` utility that implements the environment + home-file lookup (no persistence). Do you want me to proceed with that?
